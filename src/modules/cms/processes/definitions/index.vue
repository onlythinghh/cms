...
import CreateDiagramModal from '@/modules/flowable/components/create-diagram'
...
export default {
    ...
    computed: {
        ...mapState('cms/processes/definitions', ['definstions', 'deploymentId']),
        _deploymentId: {
            get() {
    ...
    methods: {
        ...mapMutations('cms/processes/definitions', ['setDeploymentId']),
        ...mapActions('cms/processes/definitions', [
            'getProcessesDefinitions',
        ...
        reloadData() {
            ...
            this.getProcessesDefinitions(params).then(() => {
                showLoading(false)
            }).catch(error => {
                showLoading(false)
        ...
        onDownloadDefinitionResource(definition) {
            ...
            let resourceName = definition.resourceName
        ...
        loadAssigneees(isShowLoading) {
            if (this.candidateUsers.length > 0 || this.candidateGroups.length > 0) {
                return Promise.resolve('')
            }
            isShowLoading && showLoading('正在加载信息')
            return this.getAssigneees().then(data => {
                isShowLoading && showLoading(false)
                this.candidateUsers = data.candidateUsers
        ...
        onUpgradeProcessDiagram(processDefinition) {
            showLoading('正在获取资源...')
            Promise.all([this.loadAssigneees(), this.getProcessDefinitionXMLResource(processDefinition.id)]).then(([assingees, xml]) => {
                this.diagram = xml
                this.diagramVisible = true
        ...
        onDeleteDeployment(definition) {
            this.$confirm('确认删除吗?', '提示', {
                ...
                this.deleteRepositoryDeployment(definition.id).then(() => {
                    showLoading(false)
