<template>
    <div class="investment">
        ...
        <el-table ref="tableList" :data="dataList" class="tableList">
                ...
                <el-table-column label="中宏股票型投资账户" prop="UnitPrice1">
                    <template v-slot="{ row }">
                        {{row.UnitPrice1 | tofixed}}
                    </template>
                </el-table-column>
                <el-table-column label="中宏混合偏股型投资账户" prop="UnitPrice2">
                    <template v-slot="{ row }">
                        {{row.UnitPrice2 | tofixed}}
                    </template>
                </el-table-column>
                <el-table-column label="中宏债券型投资账户" prop="UnitPrice3">
                    <template v-slot="{ row }">
                        {{row.UnitPrice3 | tofixed}}
                    </template>
                </el-table-column>
                <el-table-column label="中宏货币型投资账户" prop="UnitPrice4">
                    <template v-slot="{ row }">
                        {{row.UnitPrice4 | tofixed}}
                    </template>
                </el-table-column>
                <el-table-column label="中宏灵活配置型投资账户" prop="UnitPrice5">
                    <template v-slot="{ row }">
                        {{row.UnitPrice5 | tofixed}}
                    </template>
                </el-table-column>
        ...
        >
            <el-form ref="changeRef" label-width="100px" :rules="formDataRules" :model="currentRow">
                ...
                <el-form-item label="中宏股票型投资账户" prop="UnitPrice1">
                ...
                <el-form-item label="中宏混合偏股型投资账户" prop="UnitPrice2">
                ...
                <el-form-item label="中宏债券型投资账户" prop="UnitPrice3">
                    <el-input v-model="currentRow.UnitPrice3" placeholder="请输入"
                        oninput="value=value.replace(/\D*(\d*)(\.?)(\d{0,10})\d*/,'$1$2$3').replace(/^0+(\d)/, '$1').replace(/^\./, '0.').match(/^\d*(\.?\d{0,10})/g)[0] || ''"
                        style="width: 300px"
                    />
                </el-form-item>
                <el-form-item label="中宏货币型投资账户" prop="UnitPrice4">
                    <el-input v-model="currentRow.UnitPrice4" placeholder="请输入"
                        oninput="value=value.replace(/\D*(\d*)(\.?)(\d{0,10})\d*/,'$1$2$3').replace(/^0+(\d)/, '$1').replace(/^\./, '0.').match(/^\d*(\.?\d{0,10})/g)[0] || ''"
                        style="width: 300px"
                    />
                </el-form-item>
                <el-form-item label="中宏灵活配置型投资账户" prop="UnitPrice5">
                    <el-input v-model="currentRow.UnitPrice5" placeholder="请输入"
                        oninput="value=value.replace(/\D*(\d*)(\.?)(\d{0,10})\d*/,'$1$2$3').replace(/^0+(\d)/, '$1').replace(/^\./, '0.').match(/^\d*(\.?\d{0,10})/g)[0] || ''"
                        style="width: 300px"
                    />
                </el-form-item>
...
export default {
    ...
    data() {
        return {
            ...
            formDataRules: { // 表单验证规则
                ...
                UnitPrice5: [{required: true, message: "该项为必填项", trigger: "blur"}]
            },
        }
    },
    watch:{
    ...
    mounted(){
        console.log('成长星data',this.dataList)
    },
    methods: {
        ...
        changeReateRow(row, index=null) {
            if(Object.keys(row).length){
                ...
                this.currentRow = {
                    ...
                    FndVers5: "",
                    UnitPrice1: "", // 股票型投资账户
                    UnitPrice2: "",// 混合偏股型投资账户
                    UnitPrice3: "",// 债券型投资账户
                    UnitPrice4: "",// 货币型投资账户
                    UnitPrice5: "",// 灵活配置型投资账户
                    ValnDate: "", // 资产评估日期
                }
            }
            this.rowIndex = index
