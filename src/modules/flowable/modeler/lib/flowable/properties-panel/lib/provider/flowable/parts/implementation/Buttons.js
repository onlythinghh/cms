...
var elementHelper = require('../../../../helper/ElementHelper'),
    buttonsHelper = require('../../../../helper/ButtonsHelper'),
...
function getPropertyValues(parent) {
    var properties = parent && getUserTask(parent);
 ...
 */
function getUserTask(element) {
    if (!isUserTask(element)) {
        return element.properties;
    } else {
        return getUserTaskInsideExtensionElements(element);
    }
}
 ...
 * business object.
 *
 * @param {ModdleElement} userTask
 *
 * @return {ModdleElement} a flowable:Buttons object
 */
function getUserTaskInsideExtensionElements(userTask) {
    return find(userTask.values, function (elem) {
...
module.exports = function (element, bpmnFactory, options, translate) {
    ...
    assign(options, {
        ...
        addElement: function (element, node) {
            ...
            var properties = getUserTask(parent);
            if (!properties) {
                properties = elementHelper.createElement('flowable:Buttons', {}, parent, bpmnFactory);

                if (!isUserTask(parent)) {
        ...
        removeElement: function (element, node, idx) {
            ...
            if (propertyValues.length === 1) {
                // remove flowable:Buttons if the last existing property has been removed
                if (!isUserTask(parent)) {
                    ...
                    forEach(parent.values, function (value) {
                        if (is(value, 'flowable:Buttons')) {
                            commands.push(buttonsHelper.removeEntry(bo, element, value));
                        }
                    });
                }
            }
